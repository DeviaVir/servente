...
func TestShowService(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid ID", "/service/1", http.StatusOK, []byte("A fake service owned by golang...")},
		{"Non-existent ID", "/service/2", http.StatusNotFound, nil},
		{"Negative ID", "/service/-1", http.StatusNotFound, nil},
		{"Decimal ID", "/service/1.23", http.StatusNotFound, nil},
		{"String ID", "/service/foo", http.StatusNotFound, nil},
		{"Empty ID", "/service/", http.StatusNotFound, nil},
		{"Trailing slash", "/service/1/", http.StatusNotFound, nil},
	}
...
func TestCreateService(t *testing.T) {
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		code, headers, _ := ts.get(t, "/service/new")
		if code != http.StatusSeeOther {
			t.Errorf("want %d; got %d", http.StatusSeeOther, code)
		}

		if headers.Get("Location") != "/user/login" {
			t.Errorf("want Location header with content %s; got %s", "/user/login", headers.Get("Location"))
		}
	})
	...
	t.Run("Authenticated", func(t *testing.T) {
		...
		code, _, body := ts.get(t, "/service/new")
		if code != 200 {
			t.Errorf("want %d; got %d", 200, code)
		}

		formTag := "<form action='/service/new' method='POST'>"
